...
function Gradient_to_Normal(G::Array{Float64, 3})
    ...
    =#
    # Let's try this one:
    #Out[1,1] = (-G[1,1,2]-G[1, 1,1])
    #Out[1,m] = ( G[1,m,2]-G[1, m,1])
    #Out[n,1] = (-G[n,1,2]+G[n, 1,1])
    #Out[n,m] = ( G[n,m,2]+G[n, m,1])
    return Out
end
...
function sqr_boundary_coordinates(x_dim::Int64=100,y_dim::Int64=100)
    x = ((1:x_dim) .- 1)/(x_dim - 1)
    y = ((1:y_dim) .- 1)/(y_dim - 1)
    x0 =  [[1.999*x[i]-1, -0.999] for i in 1:length(x)]
    x1 =  [[1.999*x[i]-1, 0.999] for i in 1:length(x)]
    y0 =  [[-0.999, 1.999*y[i]-1] for i in 1:length(y)]
    y1 =  [[0.999, 1.999*y[i]-1] for i in 1:length(y)]
